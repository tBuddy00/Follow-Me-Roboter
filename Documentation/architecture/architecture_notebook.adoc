...
Wir trafen grundsätzlich die Annahme das unser Fahrroboter auf einem OS wie ROS II laufen muss, da dies bereits vorhandene Funktionen mit sich bringt und wir demnach weniger operativen Aufwand als Gruppe hätten.  

Abhängigkeiten in unserem Projekt waren grundlegend bei den alten Schnittstellen des Raspberry Pi und der Kameraschnittstelle. So auch erhielten wir das benötigte Batteriepakt zur Inbetriebnahme des Fahrroboters erst zu einem späteren Zeitpunkt und so zögerte sich die Testung allgemein hinaus.

Aufgrunddessen das wir einen Alphabot 2 und der Komplexität der Bilderkennungsprozessen, müssen wir die Leistungsfähigkeit des Raspberry Pi im Blick behalten und dahingehend sind wir eingeschränkt auf diese Plattform.
...
Aufgrunddessen das unser Auftraggeber einen großen Freiraum in der Gestaltung unserer Architektur gelassen hat, finden sich hier nun keine weiteren Informationen wieder.
