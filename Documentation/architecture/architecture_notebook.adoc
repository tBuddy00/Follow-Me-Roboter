...
. Wir entscheiden uns auf Wunsch des Auftraggebers für ROS II als unser Software-Framework, da es eine solide Grundlage für den Aufbau komplexer Robotic-Systeme bietet. Dank seines modularen Aufbaus können wir unseren Programmcodes flexibel und variabel einsetzen.

. Bei der Versionsauswahl fiel unsere Entscheidung auf ROS II Humble aufgrund seiner aktuellen Implementierung, der detaillierten Dokumentation und seinen vielfältigen Erweiterungsmöglichkeiten. Die neuste Version wird ausgeschlossen, da sie aufgrund mangelnder Paketverfügbarkeit nicht förderlich wäre.

. Bei der Betriebssystemwahl setzen wir auf Ubuntu 22.04 Server. Dies erfolgt gemäß den Anforderungen der ROS 2 Humble Dokumentation, den Hardwarebeschränkungen des Raspberry Pi und der umfassend vorhanden Dokumentation dieser Version.  

. Das Ubiquity Robotics Raspberry Pi Image ist für unser Projekt nicht geeignet. Dies resultiert aus Problemen mit der Kamera-Implementierung sowie der festgestellten Inkompatibilität vieler Packages, die aufgrund ihrer Abhängigkeit von Python 2.7 nicht genutzt werden können.
