...
2. calculate_movement_variable_time(self, base_rpm, angle, move):
   Verarbeitet die von der calculate_speed_variable_time Funktion Berechneten Bewegungsparameter und gibt die gewünschten Drehzahlen sowie die Bewegungsdauer für die Motoren zurück.
...
4. move_PWMB(self, speed_percent)
   Diese Funktion ist ähnlich wie move_PWMA, wird jedoch verwendet, um die Geschwindigkeit des rechten Motors einzustellen.
