...
Die Bedienung des Alphbots wird so leicht wie möglich und auf Grundlage unseres Guides gestaltet, so das keine großen Vorkenntnisse benötigt werden. Ebenso gibt es eine klare und gut strukturierte Dokumentation welche das erlernen der Software erleichtert. Die Dokumentation wird in deutscher Sprache durchgeführt, jedoch werden die Kommentare im Programmcode auf englisch sein.
